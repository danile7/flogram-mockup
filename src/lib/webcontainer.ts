...
} from './store/editor_errors_store';
import { expand_path } from './store/expanded_paths';
import { file_status, is_repl_to_save, repl_name } from './store/repl_id_store';
 ...
 * for older repositories and also if that function ever changes we would not replace that anymore.
 */
...
export const webcontainer = {
	...
	async mount_files() {
		...
		// avoid ghost files from previous repositories
		close_all_tabs();
		await tick();
