...
<script lang="ts">
	...
	import { base_path as base_path_store } from '$lib/store/base_path_store';
	import { expand_path, expanded_paths, toggle_path } from '$lib/store/expanded_paths';
	import { layout_store } from '$lib/store/layout_store';
	import { repl_name } from '$lib/store/repl_id_store';
...
<ul
	class="file-tree bg-back-1-light dark:bg-back-1-dark text-text-1-dark dark:text-text-1-light"
...
>
	{#if base_path === $base_path_store}
		<li class="root p-2 flex items-center">
			<label for="project_name" class="text-base !flex items-center text-text-3-light ">
				<Folder />
			...
			<input
				id="project_name"
				class="min-w-5 py-1 text-base border border-gray-400 border-opacity-0 hover:border-opacity-100 focus:border-primary focus:border-opacity-100 whitespace-nowrap overflow-ellipsis flex-grow [font:inherit] h-full bg-transparent transition-all duration-200"
				bind:value={$repl_name}
			/>

			<div class="flex items-stretch" class:force={$open_menus[$base_path_store]}>
	...
	{#each nodes as node_name}
				...
				{:else}
					...
					>
						<IconamoonArrowDown2Duotone
							style="transition: 0.15s ease; transform: {expanded
								? 'rotate(0deg)'
								: 'rotate(-90deg)'};"
						/>
		...
		{:else}
			...
			{@const path = base_path + node_name}
			<li
				class={` relative pl-[25px] flex items-center transition-all duration-300 ${$current_tab === path ? "bg-back-4-light dark:bg-back-3-dark":  ""}`}
				use:draggable={path}
			>
				{#if renaming_path === path}
					<AddFile
					...
					/>
				{:else}
					<button class="node border border-red-500" on:click={() => open_file(path)}>
					...
					<div class="hover-group bg-back-3-light dark:bg-back-2-dark rounded-md shadow-md">
						<button
							title="Rename"
							on:click={() => {
								renaming_path = path;
	...
	<svelte:fragment slot="dialog-actions">
		...
		<button
			...
			on:click={() => {
				...
				file_updating_store.set_delete_path(`${base_path.replace('./', '')}${deleting_file?.name}`)
				deleting_file = null;
			}}
			>Yes
		</button>
